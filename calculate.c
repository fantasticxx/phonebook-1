...
int main(void)
{
	...
	double avg_orig_a = (orig_sum_a / 100.0);
	double avg_opt_a = (opt_sum_a / 100.0);
    double avg_orig_f = (orig_sum_f / 100.0);
	double avg_opt_f = (opt_sum_f / 100.0);
	fprintf(output, "append() %lf %lf\n",avg_orig_a, avg_opt_a);
    fprintf(output, "findName() %lf %lf\n", avg_orig_f, avg_opt_f);
    fprintf(output, "total() %lf %lf",avg_orig_a + avg_orig_f, avg_opt_a + avg_opt_f);
	fclose(output);
    fclose(fp);
    return 0;
}
